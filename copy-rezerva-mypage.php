...
<?php
  ...
  if (isset($_POST['upload'])) {
  	...
  	$sql = "INSERT INTO images (image, image_text, data, userID, likes) 
  		VALUES ('$image', '$image_text', '0000-00-00 00:00:00', $userid, 0)";
  	// execute query
  	mysqli_query($db, $sql);
...
while ($row = mysqli_fetch_array($result)) {
    echo "<tr><td>";
		...
		if ($likeButt == 0){
			...
			<input type="submit" value="LIKE" name="likebutton" class="btn btn-outline-primary"/>  ';
			...
			echo '<br/><form action="like.php" class="container" method="post">
			<input name="numarIMG" type="hidden" value="' .($_POST['numarIMG'] = $imagesid).'">
			<input type="submit" value="DISLIKE" name="dislikebutton" class="btn btn-outline-danger"/>  ';
			//poate face dislike
		...
		while($row = mysqli_fetch_object($result1))
		{
			echo '<span class="border border-light">'.$row->likes.'</span></div></form>';
		}
		...
		if($delButt == 1){
			echo '<form action="delete.php" class="container" method="post">
			<input name="numarIMG" type="hidden" value="' .($_POST['numarIMG'] = $imagesid).'">
			<input type="submit" value="X" class="btn btn-outline-danger" name="deleteImg">
			</form>';
			/*
		...
		echo '
		<form action="insertcomment.php" class="content" method="post">
		<h3>Lasa un comentariu:</h3>
			<input name="numarIMG" type="hidden" value="' .($_POST['numarIMG'] = $imagesid).'">
			<textarea id="text" cols="30" rows="1" name="imgComment" placeholder="Scrie un comentariu..."></textarea>
...
/*
//<!---------INCEPUT PARTE DE LIKE/DISLIKE------->
		$like1 = mysqli_query($db, "SELECT * FROM likes WHERE imageID='$imagesid' AND userID='$userid'");

		if ($like1){
			echo '<br/><form action="#" class="container" method="post">
			<input type="submit" value="DISLIKE" name="dislikebutton" class="btn btn-outline-danger"/>  ';
			//poate face dislike
			if (isset($_POST['dislikebutton'])) {
				$like = mysqli_query($db, "SELECT * FROM images WHERE imageID=$imagesid");
				...
				if($row['imageID']==$imagesid AND $row['userID']==$userid)
					echo "DA";
				mysqli_query($db, "DELETE FROM likes WHERE imageID=$imagesid AND userID=$userid");
				mysqli_query($db, "UPDATE images SET likes=$n-1 WHERE imageID=$imagesid");
